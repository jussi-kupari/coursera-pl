  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([clsr (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? clsr)
               (letrec ([fn (closure-fun clsr)]
                        [envwithvar (cons (cons (fun-formal fn) v) (closure-env clsr))]
                        [fnname (fun-nameopt fn)]
                        [newenv (if fnname (cons (cons fnname clsr) envwithvar) envwithvar)])
